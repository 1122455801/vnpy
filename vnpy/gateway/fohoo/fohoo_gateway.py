...
DIRECTION_FOHOO2VT = {v: k for k, v in DIRECTION_VT2FOHOO.items()}
DIRECTION_FOHOO2VT[THOST_FTDC_PD_Long] = Direction.LONG
DIRECTION_FOHOO2VT[THOST_FTDC_PD_Short] = Direction.SHORT

ORDERTYPE_VT2FOHOO = {
    OrderType.LIMIT: THOST_FTDC_OPT_LimitPrice,
    OrderType.MARKET: THOST_FTDC_OPT_AnyPrice
}
ORDERTYPE_FOHOO2VT = {v: k for k, v in ORDERTYPE_VT2FOHOO.items()}
...
OFFSET_FOHOO2VT = {v: k for k, v in OFFSET_VT2FOHOO.items()}
...
class FohooGateway(BaseGateway):
    """
    VN Trader Gateway for FOHOO .
    """
    ...
    exchanges = list(EXCHANGE_FOHOO2VT.values())

    def __init__(self, event_engine):
        """Constructor"""
        super().__init__(event_engine, "FOHOO")

        self.td_api = FohooTdApi(self)
        self.md_api = FohooMdApi(self)
...
class FohooMdApi(MdApi):
    """"""

    def __init__(self, gateway):
        """Constructor"""
        super(FohooMdApi, self).__init__()
...
class FohooTdApi(TdApi):
    """"""

    def __init__(self, gateway):
        """Constructor"""
        super(FohooTdApi, self).__init__()
    ...
    def onRspOrderInsert(self, data: dict, error: dict, reqid: int, last: bool):
        ...
        order = OrderData(
            ...
            exchange=exchange,
            orderid=orderid,
            direction=DIRECTION_FOHOO2VT[data["Direction"]],
            offset=OFFSET_FOHOO2VT.get(data["CombOffsetFlag"], Offset.NONE),
        ...
        if data["InstrumentID"] in symbol_exchange_map:
            ...
            if not position:
                position = PositionData(
                    ...
                    direction=DIRECTION_FOHOO2VT[data["PosiDirection"]],
                    gateway_name=self.gateway_name
                )
    ...
    def onRspQryInstrument(self, data: dict, error: dict, reqid: int, last: bool):
        ...
        """
        product = PRODUCT_FOHOO2VT.get(data["ProductClass"], None)
        if product:
            contract = ContractData(
                symbol=data["InstrumentID"],
                exchange=EXCHANGE_FOHOO2VT[data["ExchangeID"]],
            ...
            if contract.product == Product.OPTION:
                ...
                contract.option_type = OPTIONTYPE_FOHOO2VT.get(data["OptionsType"], None)
    ...
    def onRtnOrder(self, data: dict):
        ...
        order = OrderData(
            ...
            exchange=exchange,
            orderid=orderid,
            type=ORDERTYPE_FOHOO2VT[data["OrderPriceType"]],
            direction=DIRECTION_FOHOO2VT[data["Direction"]],
            offset=OFFSET_FOHOO2VT[data["CombOffsetFlag"]],
            price=data["LimitPrice"],
            volume=data["VolumeTotalOriginal"],
            traded=data["VolumeTraded"],
            status=STATUS_FOHOO2VT[data["OrderStatus"]],
            datetime=dt,
            gateway_name=self.gateway_name
        )
        self.gateway.on_order(order)
    ...
    def onRtnTrade(self, data: dict):
        ...
        trade = TradeData(
            ...
            direction=DIRECTION_FOHOO2VT[data["Direction"]],
            offset=OFFSET_FOHOO2VT[data["OffsetFlag"]],
            price=data["Price"],
            volume=data["Volume"],
            datetime=dt,
            gateway_name=self.gateway_name
    ...
    def send_order(self, req: OrderRequest):
        ...
        Send new order.
        """
        if req.offset not in OFFSET_VT2FOHOO:
            self.gateway.write_log("请选择开平方向")
            return ""

        if req.type not in ORDERTYPE_VT2FOHOO:
        ...
        fohoo_req = {
            ...
            "OrderPriceType": ORDERTYPE_VT2FOHOO.get(req.type, ""),
            "Direction": DIRECTION_VT2FOHOO.get(req.direction, ""),
            "CombOffsetFlag": OFFSET_VT2FOHOO.get(req.offset, ""),
        ...
        if req.type == OrderType.FAK:
            fohoo_req["OrderPriceType"] = THOST_FTDC_OPT_LimitPrice
            fohoo_req["TimeCondition"] = THOST_FTDC_TC_IOC
            fohoo_req["VolumeCondition"] = THOST_FTDC_VC_AV
        elif req.type == OrderType.FOK:
            fohoo_req["OrderPriceType"] = THOST_FTDC_OPT_LimitPrice
            fohoo_req["TimeCondition"] = THOST_FTDC_TC_IOC
            fohoo_req["VolumeCondition"] = THOST_FTDC_VC_CV

        self.reqid += 1
        self.reqOrderInsert(fohoo_req, self.reqid)
    ...
    def cancel_order(self, req: CancelRequest):
        ...
        self.reqid += 1
        self.reqOrderAction(fohoo_req, self.reqid)
    ...
    def send_rfq(self, req: OrderRequest) -> str:
        ...
        self.reqid += 1
        self.reqForQuoteInsert(fohoo_req, self.reqid)
