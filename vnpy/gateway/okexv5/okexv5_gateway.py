...
PUBLIC_WEBSOCKET_HOST = "wss://wspap.okex.com:8443/ws/v5/public?brokerId=9999"
PRIVATE_WEBSOCKET_HOST = "wss://wspap.okex.com:8443/ws/v5/private?brokerId=9999"
...
TYPE_OKEXV52VT = {
    "buy": (Offset.OPEN, Direction.LONG),
    "short": (Offset.OPEN, Direction.SHORT),
    "sell": (Offset.CLOSE, Direction.SHORT),
    "cover": (Offset.CLOSE, Direction.LONG),
}
TYPE_VT2OKEXV5 = {v: k for k, v in TYPE_OKEXV52VT.items()}
...
class OkexsRestApi(RestClient):
    ...
    def send_order(self, req: OrderRequest):
        ...
        if (req.offset, req.direction) not in TYPE_VT2OKEXV5:
            return ""
        ...
        data = {
            "client_oid": orderid,
            "type": TYPE_VT2OKEXV5[(req.offset, req.direction)],
    ...
    def query_contract(self):
        ...
        for contract in contracts:
            self._query_contract(contract)

    def _query_contract(self, contract):
        data = {
            "instType": contract
        }
        self.add_request(
    ...
    def on_query_contracts(self, data, request):
        ...
        for instrument_data in data["data"]:
            symbol = instrument_data["instId"]

            product = PRODUCT_OKEXV52VT[instrument_data["instType"]]
            contract = ContractData(
                symbol=symbol,
                exchange=Exchange.OKEX,
                name=symbol,
                product=product,
                size=float(instrument_data["ctVal"]),
                pricetick=float(instrument_data["tickSz"]),
                history_data=True,
    ...
    def on_query_position(self, datas, request):
        ...
        for data in datas["data"]:
            ...
            pos = _parse_position_holding(data, symbol=symbol,
                                            gateway_name=self.gateway_name)
...
def _parse_position_holding(holding, symbol, gateway_name):
    ...
    position = int(holding["pos"])
    pos = PositionData(
        symbol=symbol,
        exchange=Exchange.OKEX,
        direction=DIRECTION_OKEXV52VT[holding['posSide']],
        volume=position,
        frozen=float(position - float(holding["availPos"])),
        price=float(holding['avgPx']),
        pnl=float(holding['upl']),
...
def _parse_account_info(info, gateway_name):
    ...
    """
    account = AccountData(
        accountid=info['ccy'].upper(),
        balance=float(info["totalEq"]),
        frozen=float(info["ordFroz"]),
        gateway_name=gateway_name,
    )
    return account
...
def _parse_order_info(order_info, gateway_name: str):
    offset, direction = TYPE_OKEXV52VT[order_info["type"]]
    ...
    order = OrderData(
        ...
        exchange=Exchange.OKEX,
        type=ORDERTYPE_OKEXV52VT[order_info["order_type"]],
        orderid=order_id,
        direction=direction,
        offset=offset,
        traded=int(order_info["fillSz"]),
        price=float(order_info["px"]),
        volume=float(order_info["sz"]),
        datetime=_parse_timestamp(order_info["uTime"]),
        status=STATUS_OKEXV52VT[order_info["state"]],
        gateway_name=gateway_name,
    )
    return order
