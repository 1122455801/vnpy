...
class FixMdApi:
    ...
    def connect(self, md_address, userid, password, brokerid):

        md_host, md_port = tuple(md_address.split(":"))
        ...
        acceptor_dict.setString("SocketAcceptHost", md_host)
        acceptor_dict.setString("SocketAcceptPort", md_port)

        acceptor_session = fix.SessionID(
            "FIX.4.2",
            SETTINGS["genus.acceptor_sender"],
            SETTINGS["genus.acceptor_target"]
        )
        ...
        initiator_session = fix.SessionID(
            "FIX.4.2",
            SETTINGS["genus.initiator_sender"],
            SETTINGS["genus.initiator_target"]
        )
...
class AcceptorApp(fix.Application):
    ...
    def __init__(self, client: "GenusClient"):
        ...
        self.callbacks: Dict[int, callable] = {
            fix.MsgType_NewOrderSingle: self.new_acceptor_order,
            fix.MsgType_OrderCancelRequest: self.cancel_acceptor_order,
        }

        self.session_id: int = 0
        self.exec_id: int = int(datetime.now().strftime("%H%M%S0000"))

        self.acceptor_orders: Dict[str, GenusacceptorOrder] = {}
